...
pub mod launchpad {
    ...
    pub fn initialize_auction(
        ...
        auction.reveal_deadline = end_time + 3600; // 1 hour after auction ends
        auction.status = AuctionStatus::Active;
        auction.clearing_price = 0;
        auction.total_bids = 0;
        auction.total_revealed = 0;
        auction.total_committed = 0;
        auction.token_supply = token_supply;
        auction.bump = ctx.bumps.auction;

        // Transfer project tokens to auction vault
        ...
        token_2022::transfer_checked(transfer_ctx, token_supply, ctx.accounts.project_mint.decimals)?;

        msg!("Auction initialized with {} tokens", token_supply);
        Ok(())
    }
    ...
    pub fn place_bid(
        ...
        // V1 MAINNET NOTE: Proof verification is performed client-side only.
        // The Noir proof is generated by the frontend and verified in the browser
        // before submission. The nullifier (derived from secret + auction_id via
        // Poseidon hash) provides Sybil resistance through PDA uniqueness.
        //
        // For V2, implement on-chain verification using:
        // - Sunspot (https://github.com/noir-lang/sunspot) for Solana
        // - Or a custom verifier program compiled from the Noir circuit
        //
        // Security model for V1:
        // - Whitelist membership: Trusted via Merkle root published by authority
        // - Double-bid prevention: Enforced via nullifier PDA uniqueness
        // - Privacy: Maintained through client-side proof generation
        msg!("V1: Proof length {} bytes (client-side verified)", proof.len());
...
pub struct InitializeAuction<'info> {
    ...
    /// Authority's project token account
    #[account(
        mut,
        token::mint = project_mint,
...
pub struct Claim<'info> {
    ...
    pub user_project_account: InterfaceAccount<'info, TokenAccount>,
