...
export async function prove(input: any, keyBasePath: string, options?: { singleThread?: boolean }): Promise<{
    proof: Proof
    publicSignals: string[];
}> {
    // In browser, snarkjs fetches .wasm and .zkey via fetch if paths are URLs
    // keyBasePath should be the prefix URL e.g. "/wasm/transaction2"
...
function yElementIsPositiveG1(yElement: BN): boolean {
    return yElement.lte(FIELD_SIZE.sub(new BN(yElement.toString())));
}
