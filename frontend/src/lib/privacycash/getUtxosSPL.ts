...
import { FETCH_UTXOS_GROUP_SIZE, RELAYER_API_URL, LSK_ENCRYPTED_OUTPUTS, LSK_FETCH_OFFSET, PROGRAM_ID, tokens } from './utils/constants';
...
export async function getUtxosSPL({ publicKey, connection, encryptionService, storage, abortSignal, offset, mintAddress }: {
    ...
    try {
        ...
        while (loops < 100) {
            ...
            let fetched;
            try {
                fetched = await fetchUserUtxos({ url: fetch_utxo_url, encryptionService, storage, publicKey_ata, tokenName: token.name })
            } catch (e) {
                logger.error('Error fetching SPL UTXOs', e as string);
                break;
            }
...
async function decrypt_outputs(
    ...
    if (results.length > 0) {
        ...
        if (!j.indices || !Array.isArray(j.indices) || j.indices.length != encrypted_outputs.length) {
            console.warn('failed fetching /utxos/indices')
        } else {
